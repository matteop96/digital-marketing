...
data_dir = "/Users/matteo/Desktop/DATA SCIENCE/Digital Marketing (Web marketing)/LAB/Dataset progetto/2020_DS_lab_digital_mrkt_data_pt1"
...
#Osservo nel dettaglio i clienti che possiedono un numero di sottoscrizioni tessere fedeltà pari a 3.
...
plot_fedeltà_dist_last_date #è possibile osservare un picco di sottoscrizioni al programma fedeltà il giorno 23 novembre 2018. Nei primi mesi di novembre vi è stata una sostanziale riduzione del numero di sottoscrizioni al programma fedeltà.
...
#tramite le operazioni precedenti, osservo come TYP_JOB essendo un'informazione non raccolta, non presenta dei valori. E' stato inserito il valore 0 in W_PHONE così da andare a dire che quell'informazione non è stata raccolta.
account_clean
...
consistency_idcli_fidelity_account <- fedeltà_clean %>%
  ...
  as.data.frame()
#Possiamo quindi constatare che tutti gli ID_CLI nel dataset fedeltà, 
...
plot_account_dist_email_provider #vi è un numero elevato di email provider, tanto che a livello grafico si nota decisamente questo numero molto elevato. Non conviene quindi pensare di trasforare in fattoriale anche l'email-provider. Piuttosto seguiranno delle analisi successive. Scorrendo brevemente si nota un possibile errore ad esempio nell'email provider g.mail.it.
...
plot_account_dist_typ_cli_account_liberip <- (
  ggplot(data=account_dist_typ_cli_account_liberip
         , aes(x=TYP_CLI_ACCOUNT, y=TOT_CLIs)) +
    geom_bar(stat="identity"
             , fill="red") +
    theme_minimal() + coord_flip()
)
plot_account_dist_typ_cli_account_liberip #si osserva che tutti i liberi professionisti presentano un valore pari a 4 come account tipologia cliente.
...
address_clean %>%
  summarize(TOT_ID_ADDRESSes = n_distinct(ID_ADDRESS)
            , TOT_ROWs = n())
#vi è la presenza di duplicati, considerando la presenza di circa 3 volte tanto di row rispetto agli indirizzi. Probabilmente, poi verificherò, le ripetizioni sono dovute a clienti che possiedono più programmi fedeltà e che quindi la registrazione dell'indirizzo è avvenuta più volte nel database.
...
address_clean %>% filter(!is.na(PRV) & is.na(REGION)) #si può immediatamente notare come alcune provincie non siano scritte in una maniera comune, inteso alcune sono scritte con il nome completo della provincia, altre con il nome abbreviato. 
...
address_dist_region <- address_clean %>%
  group_by(REGION) %>%
  summarize(TOT_IDs = n_distinct(ID_ADDRESS)) %>%
...
plot_address_dist_region #si può chiaramente notare come la maggior parte di clienti sono della regione lombardia, seguita da altre regioni come: Veneto, Sicilia, Lazio, Campania. Non sembra esserci una correlazione di gruppo regionale, inteso come Nord, Centro e Sud. 
...
campagnia_categoria_clean %>%
  summarize(TOT_ID_CAMPs = n_distinct(ID_CAMP)
...
str(df_6_camp_event) #molte informazioni sono racchiuse in questo dataset. La data dell'evento necessiterà delle leggere modifiche sicuramente. L'ID_CLI riguarda l'azione dell'evento a quale cliente è associata. 
...
consistency_idcli_fidelity_event #si può osservare che nel dataset 6 mancano 1667449 clienti che invece sono riportati nel dataset 1.
...
plot_tic_dist_importosconto_id_cli <- ggplot(data = tic_dist_importosconto_id_cli,
       aes(x = ID_CLI,
           y = IMPORTO_LORDO)) +      
 geom_bar(stat = "identity") + 
 theme_minimal()
plot_tic_dist_importosconto_id_cli #da plottare ma limitare tipo ai primi 15 valori.
...
monetary_classi #elevato numero di medium:
...
rfm_dataset_finale
rfm_dataset_finale_classi <- as.data.frame(table(rfm_dataset_finale$RF))
...
rf_dataset_ <- as.data.frame(table(rfm_dataset_finale$RF))
rf_dataset_
plot_rf_dataset_ <- ggplot(data = rf_dataset_,
       aes(x = Var1, y = Freq,
           fill = Freq)) +                        
  geom_bar(stat = "identity") +                   
  scale_colour_brewer(palette = "Spectral") +
  ...
  scale_x_discrete(labels = c("Engaged", "Leaving", "Leaving Top",
                              "One Timer", "Top")) + 
  guides(fill = FALSE)
plot_rf_dataset_ #molto chiara la visualizzazione, si nota la prevalenza di...
...
plot_rfm_dataset_ <- ggplot(rfm_dataset_, aes(x = RF, y = Monetary, fill = Value)) + 
  ...
  scale_fill_distiller(palette = "Spectral") +
  theme_minimal()
plot_rf_dataset_ #costruito quindi il barplot
rfm_classi_dataset <- as.data.frame(table(rfm_dataset_finale$RFM))
rfm_classi_dataset
plot_rfm_finale <- ggplot(data = rfm_classi_dataset,
       aes(x = Var1, y = Freq,
           fill = Freq)) +                        
  geom_bar(stat = "identity") +                   
  scale_colour_brewer(palette = "Spectral") +
...
//SALVARE DATASETS//
```{r}
save(rfm_dataset_finale, monetary_dev, recency_dev, frequency_dev, file = "rfm.RData")
```
#CALCOLO CHURN RATE#
```{r}
#FRASI DI COMMENTO:
...
table(churn$CHURN) #eseguire rappresentazione
```
#definire una serie di variabili#
* RECENCY;
* SPESA;
* TOT_PURCHASE;
* REGION;
...
#REGRESSION TREES#
```{r}
regression_tree <- rpart(CHURN ~ RECENCY + SPESA + TOT_PURCHASE + REGION + TYP_JOB, data = train)
...
```{r}
memory.limit(100000)
random_forest_tree <- randomForest(CHURN ~ RECENCY + SPESA + TOT_PURCHASE + REGION + TYP_JOB, data = train, ntree = 100)
print(random_forest_tree)
```
#REGRESSION LOGISTICA#
```{r}
regressione_logistica <- train(CHURN ~ RECENCY + SPESA + TOT_PURCHASE + REGION + TYP_JOB, data = train, method = "glm")
...
```{r}
plot_accuracy <- ggplot(data = accuracy,
       aes(x = Models,
           y = Accuracy,
           fill = Models)) +
  geom_bar(stat = "identity") +
...
```{r}
#Probability
p_tree = predict(regression_tree, test[,-5], "prob")[,1]
p_rf = predict(random_forest_tree, test[,-5], "prob")[,1]
p_log = predict(regressione_logistica, test[,-5], "prob")[,1]
...
```{r}
#Mi riferisco sempre alla solita data scelta
...
previsione$prob_to_churn <- predict(regressione_logistica, previsione, type = "prob")[,2]
previsione
```
#R BASKET ANALYSIS#
```{r}
#Best sellers:
count_tic <- tic_clean_finale %>%
  group_by(ID_ARTICOLO) %>%
  summarise(count = n()) %>%
  arrange(desc(count)) %>%
...
#Association Rule:
tic_ordered <- tic_clean_finale[order(tic_clean_finale$ID_CLI),]
lista <- plyr::ddply(tic_clean_finale, c("ID_CLI", "TIC_DATE"),
                         function(df1)paste(df1$ID_ARTICOLO, 
                       collapse = ","))
lista$ID_ARTICOLO <- NULL
lista$TIC_DATE <- NULL
colnames(lista) <- c("items")

write.csv(lista, file.path(data_dir, "market_basket.csv"),
          quote = FALSE, row.names = TRUE)
```

#conclusione market basket analysis:
```{r}
tr <- arules::read.transactions('market_basket.csv', format = 'basket', sep=',') #concludere
```
